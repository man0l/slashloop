...
import { formatCompactViews, sourceTag } from './service.js';
...
describe('experiment title differentiator',()=>{
  test('compact views',()=>{expect(formatCompactViews(8200000)).toBe('8.2M');expect(formatCompactViews(1300000)).toBe('1.3M');expect(formatCompactViews(961600)).toBe('961.6k');expect(formatCompactViews(75600)).toBe('75.6k');expect(formatCompactViews(999)).toBe('999');});
  test('source tag carries handle, caption snippet and views',()=>{expect(sourceTag({creatorHandle:'ballymaybach',caption:'prove me wrong',views:75600})).toBe('@ballymaybach · prove me wrong (75.6k)');expect(sourceTag({creatorHandle:'chris.creates.ai',caption:'Offical scores with @PSL App #facialfeatures #beauty #testosterone #fyp ',views:1300000})).toContain('@chris.creates.ai · ');});
  test('untitled fallback when caption is empty',()=>{expect(sourceTag({creatorHandle:null,caption:'',views:null})).toBe('untitled · untitled');});
});
