...
1. ~~Hook tests v1 (text-only)~~ **SHIPPED (2026-08-24, MCP + standalone site UI)** — HookTest/HookVersion objects, `start_hook_test` + `reroll_hooks` (2 cr each, one Gemini call apiece; insight/same-in/beats are the editable lock re-rolls obey), `pick_hook_versions` by label or ID, `export_shotlist` markdown, gallery 🧪 badges + "Has hook test" filter (MCP tool arg, signed-URL param, HTML toolbar). Site parity same day: start dialog + full test panel from gallery cards and a `/tests` manager over the same REST actions. Render (`/render`) and own-post matching stay Phase 3.
